    ...
    $.ajax({
      ...
      myfilter = {
        ...
        $.ajax({
          ...
          $("#cardresults").empty();
          for (i in results) {
            var startdate = new Date(results[i].starttime);
            var LOCALTIME = startdate.toLocaleString("en-US", {timeZone: "America/Chicago"})
            var datetime = LOCALTIME.split(",");
            var date = datetime[0];
            var timestring = datetime[1].split(":");
            var modifier = timestring[2].split(" ");
            var time = timestring[0]+":"+timestring[1] + " " + modifier[1]
            ...
            let infowindow = new google.maps.InfoWindow({
              content:
                ...
                "<p><bold>Distance: </bold>" + results[i].distance.toFixed(2) + " miles</p>" +
            ...
            var gametime = $("<p>").html("<strong>When: </strong>" + date + " at " + time);
